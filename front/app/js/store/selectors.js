...
import {hasLength, idMatchObjId, listMatchObjId, noOp, stripHTML} from '../utils/AppUtils';
...
export const getEnrollmentRecordLMSCourses = (userEnrolledCourses, courseLMSIds) =>
    userEnrolledCourses
        .filter(listMatchObjId(courseLMSIds));
...
export const getHydratedContent = () => {
    ...
    return content.reduce((acc, contobj) => {
        ...
        if (lmsEnrollments.length > 0) {
            // If there is no record then you are not enrolled, otherwise you must
            // be incomplete or complete. Not started isn't available
            let statusKeys = lmsEnrollments.map(enrollment => enrollment.status.completionstatus || enrollment.status);
            let prioritizedStatus = statusKeys.find(status => status.completed);
            o.lmsStatus = prioritizedStatus ? 2 : 1;

            //console.log(lmsEnrollments.id, statusKey, o.lmsStatus)

            if (o.lmsStatus === 2) {
                // Totara dates are seconds since 1/1/70 12:00am
                o.lmsStatusDate = moment(new Date(parseInt(prioritizedStatus.completions[0].timecompleted * 1000)));
                o.isComplete = true;
            }
        }
